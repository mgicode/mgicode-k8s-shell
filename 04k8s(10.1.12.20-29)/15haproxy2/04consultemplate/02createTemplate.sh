...
echo -e "\033[32m ###############/consulTemplate/haproxyTemplateConfig${ip##*.}  ########### \033[0m"
cat /consulTemplate/haproxyTemplateConfig${ip##*.} 
...
# {{ range service "sys-gateway-debug" }} server {{ .Name }}{{ .Address }}:{{ .Port }}check inter 2000 rise 3 fall 3 weight 30 {{ end }}

#  {{range $key, $pairs := tree "hello/" | byKey}}{{range $serverid, $pair := $pairs}}
#   server app {{.Value}} check inter 2000 fall 3 weight 1 {{end}}{{end}}

# {{ range service "sys-gateway-debug" }}
