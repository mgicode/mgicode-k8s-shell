#ETCD  10.1.12.206 (先采用物理机的方式安装，之后采用容器实现）
...
mkdir -p /working/
cd /working/
wget http://${FILE_SERVER_IP}/etcd-v3.1.6-linux-amd64.tar.gz
...
etcdctl --endpoints=https://${NODE1_IP}:2379 member list
