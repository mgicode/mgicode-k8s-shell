 ...
 cat > /etc/docker/daemon.json << EOF
{
  ...
  "insecure-registries":["10.1.12.201:5000","10.1.11.60:5000"] 
}
EOF
...
#6、把ip修改为10.1.12.251 克隆为模板
sed -i "s/10.1.12.200/10.1.12.251/g" /etc/sysconfig/network-scripts/ifcfg-ens160
service network restart

#7 把ip修改过来
sed -i "s/10.1.12.251/10.1.12.200/g" /etc/sysconfig/network-scripts/ifcfg-ens160
service network restart
ip addr
...
#docker pull 10.1.11.60:5000/nginx:1.7.9

mkdir /www
#把相关的文件放到宿主机的/www中，即可直接访问
docker run -d -v  /www:/usr/share/nginx/html --restart=always \
  --name nginx-server -p 80:80  10.1.12.201:5000/nginx:1.7.9 
  #put aa.txt into /www

  curl http://10.1.12.200/aa.txt
